...
function CommentBody({ text, isDeleted, lines = 4 }) {
  ...
  if (!has) return null
  if (isDeleted) return <T>Comment deleted</T>
  ...
  return (
    <>
      <div
        ...
        className={clsx(
          'comment__textClamp',
          !open && (lines === 3 ? 'comment__textClamp--3' : 'comment__textClamp--4')
        )}
      >
      ...
      {canToggle || open ? (
        <button type="button" className="comment__more" onClick={() => setOpen((v) => !v)}>
          {open ? <T caseMode="lower">less</T> : <T caseMode="lower">more</T>}
        </button>
      ) : null}
    </>
  )
...
export default function WatchCommentsSection({ video }) {
  ...
  const showReply = useTranslate('reply')
  const showReplies = useTranslate('replies')
  const showAddComment = useTranslate('Add a comment…')
  const showSignInToComment = useTranslate('Sign in to comment')
  const showWriteReply = useTranslate('Write a reply…')
  const showEditComment = useTranslate('Edit your comment…')
  const showDelete = useTranslate('Delete this comment?')
  ...
  // optional: reset expanded threads when video changes
  useEffect(() => {
  ...
  const [modalMode, setModalMode] = useState('reply')
  ...
  const onDelete = useCallback(
    (comment) => {
      ...
      const ok = window.confirm(showDelete)
      if (!ok) return

      dispatch(deleteComment({ id, videoId }))
    },
    [dispatch, videoId, canEditDelete, showDelete]
  )
            ...
            <div className="comment-modal__card">
                  ...
                  <div className="comment-modal__previewTop">
                    ...
                    <div className="comment-modal__previewMeta">
                      ...
                      <div className="comment-modal__previewShort">
                        <CommentBody
                          key={`modal:${toId(modalTarget?._id)}`}
                          text={String(modalTarget?.text || modalTarget?.content || '')}
                          isDeleted={Boolean(modalTarget?.isDeleted)}
                          lines={3}
                        />
                      </div>
                    </div>
              ...
              <div className="comment-modal__form">
                <Input
                  as="textarea"
                  rows={1}
                  style={{ minHeight: 'unset', height: 'auto', paddingLeft: '4px', paddingRight: '4px' }}
                  placeholder={modalMode === 'reply' ? showWriteReply : showEditComment}
                  value={modalText}
    ...
    <section className="watch-comments">
      ...
      <div className="watch-comments__composer">
        ...
        <div className="watch-comments__composerMain">
          <Input
            ...
            placeholder={canComment ? showAddComment : showSignInToComment}
            value={text}
      ...
      <div className="watch-comments__list">
        ...
        {threads.map(({ root, replies }) => {
          ...
          const expanded = hasReplies ? isThreadOpen(rootId) : false
            ...
            <div key={rootId} className="comment-thread">
              ...
              <article className={clsx('comment', isPinned && 'comment--pinned')}>
                ...
                <div className="comment__main">
                  ...
                  {hasReplies ? (
                    <button
                      style={{ border: '1px solid #a1a1aa', marginTop: '6px' }}
                      type="button"
                      className={clsx('comment-thread__toggle', expanded && 'is-open')}
                      onClick={() => toggleThread(rootId)}
                    >
                      <span className="comment-thread__toggleLabel">
                        {replies.length} {replies.length === 1 ? showReply : showReplies}
                      </span>
                      <span className="comment-thread__toggleChevron" aria-hidden="true">
                        {expanded ? '▴' : '▾'}
                      </span>
                    </button>
                  ) : null}
