...
function CommentBody({ text, isDeleted, lines = 4 }) {
  ...
  // визначаємо, чи є overflow (чи потрібна кнопка more)
  useEffect(() => {
    const el = boxRef.current
    if (!el) return

    // якщо open=true — кнопку все одно показуємо, нічого не міряємо
    if (open) return

    // даємо DOM застосувати line-clamp
...
export default function WatchCommentsSection({ video }) {
  ...
  /* ================= replies collapse (YouTube-style) ================= */
            ...
            <div key={rootId} className="comment-thread">
              ...
              <article className={clsx('comment', isPinned && 'comment--pinned')}>
                ...
                <div className="comment__main">
                  ...
                  {/* ✅ YouTube-style toggle under root (only if there are replies) */}
                  {hasReplies ? (
                    <button
